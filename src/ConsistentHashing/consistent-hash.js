...
function ConsistentHash(options) {
  ...
  if (options.controlPoints || options.weight)
    this._weightDefault = options.controlPoints || options.weight;
...
ConsistentHash.prototype = {
  ...
  _concat2: function _concat2(target, array) {
    for (var i = 0; i < array.length; i++) target.push(array[i]);
    return target;
  },

  _makeControlPoints: function _makeControlPoints(n) {
    var attemptCount = 0;
    var i,
      key,
      points = new Array(n);
    for (i = 0; i < n; i++) {
      ...
      do {
        key = (randomGenerator.random() * this._range) >>> 0;
      } while (
        (this._keyMap[key] !== undefined || points[key] === "a") &&
        ++attemptCount < 100
      );
      // adding the always-false ( == 'a') test above doubles throughput ??
      if (attemptCount >= 100)
        throw new Error("unable to find an unused control point, tried 100");
   ...
   */
  _buildKeyMap: function _buildKeyMap(n) {
    ...
    for (var i = 0; i < this.nodeCount; i++)
      if (this._nodeKeys[i][0] === undefined) nodeCount += 1;
    ...
    for (var i = 0; i < this._nodeKeys.length; i++) {
      ...
      if (keys[0] === undefined) {
        ...
        keys.length = 0;
        for (var j = 0; j < this._weightDefault; j++)
          keys[j] = Math.round(offset + step * nodeCount * j);
      }
  ...
  remove: function remove(node) {
    ...
    while ((ix = this._nodes.indexOf(node)) >= 0) {
      ...
      if (this._keyMap) {
        for (var j = 0; j < this._nodeKeys[ix].length; j++)
          this._keyMap[this._nodeKeys[ix][j]] = undefined;
      }
  ...
  _getMany: function _getMany(name, n) {
    if (!this.keyCount) return null;
    var index = this._locate(name);
    var node,
      nodes = [];
...
**/
  ...
  _absearch: function _absearch(array, key) {
    var i,
      j,
      mid,
      gap = 25,
  ...
  _buildKeys: function _buildKeys() {
    var i,
      j,
      nodeKeys,
      keys = new Array();
