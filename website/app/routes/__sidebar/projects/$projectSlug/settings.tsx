import { Button, Card, CardProps, HStack, Icon, Input, Spinner, Stack } from '@chakra-ui/react';
import { useLoaderData } from '@remix-run/react';
import type { LoaderFunction } from '@remix-run/server-runtime';
import { redirect } from '@remix-run/server-runtime';
import { e, FirebaseUser, Project, ProjectSlug, UtcDate } from '@timetriggers/domain';
...
export default function () {
    const { project, user, now } = e.unsafeGetOrThrow(pipe(useLoaderData(), wireCodec.decode));
...
const CardWithPadding = (p: { children: React.ReactNode } & CardProps) => (
    <Card p={4} {...p}>
        {p.children}
    </Card>
);
...
const EditProjectName = ({ project }: { project: Project }) => {
    ...
    const checkSlugAvailable = _.debounce((slug: string) => {
        ...
        fetch(`/check-slug-available?slug=${slug}`).then((r) =>
            r.json().then((r) => setAvailable({ loading: false, available: r?.isAvailable })),
        );
    }, 300);
    return (
