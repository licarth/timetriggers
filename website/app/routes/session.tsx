...
import { FirebaseUserId, getProjectsForUser, te } from "@timetriggers/domain";
...
export const action: ActionFunction = async ({ request }) => {
  try {
    ...
    if (request.method === "POST") {
      ...
      const { uid: stringUid, email } = await auth.verifyIdToken(idToken);
      ...
      const projects = {
        ...
        can_read: userProjects
          .filter((p) => p.readerIds?.map((uid) => uid.id)?.includes(uid.id))
          .map((p) => p.id),
        can_edit: userProjects
          .filter((p) => p.editorIds?.map((uid) => uid.id)?.includes(uid.id))
          .map((p) => p.id),
      };
